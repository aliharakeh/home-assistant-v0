...
import { ArrowLeft, Plus, X, Percent, DollarSign, Calendar, AlertCircle } from "lucide-react"

import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from "@/components/ui/card"
import type { Shareholder, RentDuration } from "@/lib/data"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const homeData = {
        ...
        rentDuration,
        shareholders,
        electricityBills: isEdit && homeName ? getHomeByName(decodeURIComponent(homeName))?.electricityBills || [] : [],
      }
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 items-center">
                ...
                <div className="flex items-center gap-2 border rounded-md p-2">
                  ...
                  <RadioGroup
                    value={rentDuration}
                    onValueChange={(value) => setRentDuration(value as RentDuration)}
              ...
              <div className="grid gap-2">
                <div className="flex gap-2">
                  ...
                  <div className="flex items-center gap-1 w-[40%]">
                    ...
                    >
                      {isPercentage ? <Percent className="h-4 w-4" /> : <DollarSign className="h-4 w-4" />}
                    </Toggle>
                  </div>
                  <Button type="button" size="icon" onClick={addShareholder} className="w-[10%]">
                  ...
                  <div className="mt-2 space-y-2">
                    {shareholders.map((shareholder, index) => (
                      <div key={index} className="flex items-center gap-2 bg-muted p-2 rounded-md">
                        <span className="text-sm flex-1">{shareholder.name}</span>
                        <div className="flex items-center gap-1">
                          <Input
                            value={shareholder.amount}
                            onChange={(e) => updateShareholderAmount(index, e.target.value)}
                            type="number"
                            min="0"
                            step="any"
                          ...
                          <Toggle
                            ...
                            onPressedChange={() => toggleShareholderType(index)}
                            size="sm"
                            aria-label="Toggle percentage"
                            className="h-8"
                          >
