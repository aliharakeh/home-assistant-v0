...
import type { Home, ElectricityBill } from "@/lib/data"
...
type HomeContextType = {
  ...
  loading: boolean
  error: string | null
  getHomeByName: (name: string) => Home | undefined
...
export function HomeProvider({ children }: { children: React.ReactNode }) {
  ...
  const getHomeByName = (name: string): Home | undefined => {
  ...
  const updateHome = async (updatedHome: Home): Promise<void> => {
    try {
      ...
      setHomes((prevHomes) => prevHomes.map((home) => (home.name === updatedHome.name ? updatedHome : home)))
    } catch (err) {
  ...
  const addBillToHome = async (homeName: string, bill: Omit<ElectricityBill, "id">): Promise<void> => {
    try {
      ...
      setHomes((prevHomes) =>
        prevHomes.map((home) => {
          if (home.name === homeName) {
            return {
              ...home,
              electricityBills: home.electricityBills.filter((bill) => bill.id !== billId),
            }
          }
          return home
        }),
  ...
  const deleteBillsInDateRange = async (homeName: string, startDate: Date, endDate: Date): Promise<number> => {
    try {
      // Get the home
      const home = getHomeByName(homeName)
      if (!home) {
